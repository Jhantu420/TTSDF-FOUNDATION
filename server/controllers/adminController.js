...
const registerAdmin = async (req, res) => {
  try {
    ...
    const existAdmin = await adminModel.findOne({ mobile });
    if (existAdmin) {
      return res
        .status(400)
...
const createBranchAdmin = async (req, res) => {
  try {
    const { name, email, mobile, password, role, branchName, branchCode } =
      req.body;
    ...
    // Check if mobile already exists in userModel
    const existUser = await userModel.findOne({ mobile });
    if (existUser) {
      return res
        .status(400)
        .json({ message: "mobile is already registered as a user" });
    }
...
const createActivity = async (req, res) => {
  const requestingAdmin = await adminModel.findById(req.userId);
  if (!requestingAdmin || !["super"].includes(requestingAdmin.role)) {
